...
sed -n '1,46p' "$WORKFLOW" | rg -q '^  "contents": "read"$' \
  || fail "workflow-wide contents permission must be read"
if sed -n '1,46p' "$WORKFLOW" | rg -n 'contents.*write'; then
  fail "workflow-wide contents permission must not be write"
fi

rg -q '^  verify-release-gates:$' "$WORKFLOW" \
...
plan_block="$(awk '
  ...
  in_plan { print }
' "$WORKFLOW")"
printf '%s\n' "$plan_block" | rg -q '^      - verify-release-gates$' \
  || fail "release plan must depend on the release gate"
rg -q 'cargo install cargo-dist --version 0\.32\.0 --locked' "$WORKFLOW" \
  || fail "cargo-dist must be installed from the locked crate release"
if rg -n 'cargo-dist-installer\.sh|matrix\.install_dist\.run' "$WORKFLOW"; then
  fail "unverified/generated cargo-dist installer path remains"
fi

rg -q '^allow-dirty = \["ci"\]$' "$CONFIG" \
